...
from actions.auto_video_creator import auto_video_creator
...
from actions.network_monitor import network_status
from actions.cyber_sleep import cyber_sleep
from actions.auto_pilot import auto_pilot
...
from ui.welcome_screen import show_welcome_hologram
...
TOOL_DECLARATIONS = [
    ...
    {"name": "auto_video_creator", "description": "Çocuklar için otomatik eğitici videolar oluşturup YouTube'a yükler.", "parameters": {"type": "OBJECT", "properties": {"action": {"type": "STRING"}}}},
    ...
    {"name": "reminder", "description": "Hatırlatıcı.", "parameters": {"type": "OBJECT", "properties": {"minutes": {"type": "NUMBER"}, "message": {"type": "STRING"}}}},
    ...
    {"name": "lockdown_protocol", "description": "Sistem kilidi.", "parameters": {"type": "OBJECT", "properties": {}}},
    ...
    {"name": "auto_pilot", "description": "Otomatik pilot.", "parameters": {"type": "OBJECT", "properties": {}}},
    ...
    {"name": "game_updater", "description": "Oyun güncelle.", "parameters": {"type": "OBJECT", "properties": {"action": {"type": "STRING"}, "platform": {"type": "STRING"}, "game_name": {"type": "STRING"}, "app_id": {"type": "STRING"}}}},
    ...
    {"name": "workspace_mode", "description": "Çalışma alanı.", "parameters": {"type": "OBJECT", "properties": {}}},
...
class JarvisLive:
    def __init__(self, ui: JarvisUI, plugin: CyberEyePlugin, memory_core: Optional[JarvisMemory] = None, emoji_manager=None):
        ...
        try:
            ...
            self.behavior = BehaviorAnalyzer() if BehaviorAnalyzer else None
            self.events = EventTriggerSystem(memory_manager=self.memory_core) if EventTriggerSystem else None
        except:
    ...
    def force_speak(self, text: str):
        ...
        command = f"Lütfen şu metni kelimesi kelimesine, çok havalı, otoriter ve ciddi bir yapay zeka tonuyla sesli olarak oku. Başka hiçbir kelime ekleme: '{text}'"
        try:
            asyncio.run_coroutine_threadsafe(
    ...
    def extract_emoji_by_keyword(self, text: str) -> str:
        # Hem kelimeleri hem de direkt emoji simgelerini eşleştiriyoruz
        keyword_map = {
            "köpek": "dog", "🐶": "dog",
            "kedi": "cat", "🐱": "cat",
            "evet": "check", "✅": "check",
            "hayır": "cross", "❌": "cross",
            "güzel": "thumbs_up", "👍": "thumbs_up",
            "şaşırdım": "thinking", "🤔": "thinking",
            "ağlıyorum": "cry", "😢": "cry",
            "sinirli": "angry", "😡": "angry",
            "sevgi": "heart", "❤️": "heart",
            "mutlu": "happy", "😊": "happy",
            "korktum": "scared", "😱": "scared"
        }
    ...
    def show_emoji_from_text(self, text: str):
        if not self.emoji:
            return
        name = self.extract_emoji_by_keyword(text)
        if name:
            self.ui.root.after(0, lambda: self.emoji.show_image(name))
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "manage_xp" and self.gamification:
                action = args.get("action", "add")
                amount = int(args.get("amount", 0))
                reason = args.get("reason", "")
                if action == "add":
                    result = self.gamification.add_xp(amount, reason)
                else:
                    result = self.gamification.punish(amount, reason)
                self.speak(result)
                ...
                result = await loop.run_in_executor(None, lambda: kod_rehberi(parameters=args, player=self.ui, root=self.ui.root))
            ...
            elif name == "auto_video_creator":
                result = await loop.run_in_executor(None, lambda: auto_video_creator(parameters=args, player=self.ui, speak=self.speak))
                ...
                result = await loop.run_in_executor(None, lambda: hologram_hud(parameters=args, player=self.ui, root=self.ui.root))
                    ...
                    if server_content:
                        if getattr(server_content, "input_transcription", None):
                            ...
                            if transcript and not _should_skip_text(transcript):
                                ...
                                self.show_emoji_from_text(transcript)
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            self.ui.write_log(f"SYS: Mikrofon donanım hatası -> {exc}")
            raise
        finally:
            await asyncio.to_thread(stream.close)

    async def _play_audio(self):
        stream = await asyncio.to_thread(pya.open, format=FORMAT, channels=CHANNELS, rate=RECV_RATE, output=True)
        try:
            while True:
                ...
                if self.audio_in_queue.empty():
                    ...
                    if time.time() - self.last_audio_time >= 0.25:
                        self.is_speaking = False
...
def main():
    ...
    try:
        from ui.emoji_manager import EmojiManager
        emoji_manager = EmojiManager(ui.root, emoji_size=64)
    except ImportError:
        print("SYS: EmojiManager bulunamadı, emojiler devre dışı.")
        emoji_manager = None
        
    show_welcome_hologram(ui.root)
    ...
    def shutdown_app():
        if not memory_core.verify_shutdown_pin():
            return False
        try:
            plugin.stop()
            consciousness.stop()
