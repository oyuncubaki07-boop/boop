...
def desktop_control(parameters=None, player=None) -> str:
    ...
    if player: player.write_log(f"SYS: 🖥️ Masaüstü Yöneticisi -> {action}")
    ...
    try:
        if action == "wallpaper":
        ...
        elif action == "clean" or action == "organize":
            ...
            for item in desktop_path.iterdir():
                # Kısayolları (.lnk) ve sistem dosyalarını (desktop.ini) gizli tutarak diğerlerini arşive atar
                if item.is_file() and item.suffix != ".lnk" and item.name != "desktop.ini":
