...
MODEL_WRITER       = "gemini-2.5-flash" # İstersen flash-lite olarak bırakabilirsin ama 2.5-flash daha zeki kod yazar tatlım.
...
def _get_api_key() -> str:
    try:
        ...
        print(f"[DevAgent] API key load error: {e}")
        return ""
...
def _build_project(
    ...
    def log(msg: str):
        ...
        if player:
            player.write_log(f"SYS: [DevAgent] {msg}")
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output):
            msg = (
                f"Proje '{proj_name}' çalışıyor patron. "
                f"{attempt} denemede başarıyla inşa ettim. "
                f"VSCode üzerinden {project_dir} dizininde açıldı."
            )
            if speak: speak(msg)
            return f"{msg}\n\nOutput:\n{last_output}"
    ...
    msg = (
        f"'{proj_name}' projesini {MAX_FIX_ATTEMPTS} hata ayıklama denemesi sonucunda tam olarak çalışır hale getiremedim patron. "
        f"Ancak proje {project_dir} konumunda kayıtlı ve VSCode'da açık, manuel olarak inceleyebilirsin."
    )
    if speak: speak(msg)
    return f"{msg}\n\nLast error:\n{last_output[:500]}"
...
def dev_agent(
    ...
    if not description:
        return "Lütfen inşa etmemi istediğiniz projeyi açıklayın patron."

    if player: player.write_log("SYS: 🤖 DevAgent başlatıldı. Proje analiz ediliyor...")
